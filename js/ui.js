...
export let state = { currentUser: null, sessions: [], authError: null, firestoreError: null, isDemoMode: localStorage.getItem('is-demo-mode') === 'true' };
...
export async function updateSyncIndicator() {
    ...
    if (!networkOnline) {
        ...
        if (queued > 0) {
            ...
            const buttonText = state.isDemoMode ? 'Exit Demo' : 'Demo Mode';
            ind.innerHTML = `<span class="text-[10px] font-bold text-amber-500">${buttonText}</span>`;
            ind.dataset.reason = 'demo';
            ind.title = state.isDemoMode ? 'Click to clear demo data' : 'Click to load demo data';
        }
    }
...
function generateMockData() {
    const sessions = [];
    const types = ['Practice', 'Live Wrestling', 'Drilling', 'Conditioning'];
    const now = Date.now();
    const startDate = now - 365 * 24 * 60 * 60 * 1000; // 1 year ago
    let totalMinutes = 0;
    const targetMinutes = 350 * 60; // Team Captain level
    
    while (totalMinutes < targetMinutes) {
        const date = startDate + Math.random() * (now - startDate);
        const type = types[Math.floor(Math.random() * types.length)];
        const duration = 60 + Math.random() * 60; // 60-120 min
        const intensity = 1 + Math.floor(Math.random() * 10);
        const notes = getRandomNote(type);
...
function getRandomNote(type) {
    const noteOptions = {
        'Practice': [
            'Worked on takedowns and escapes',
            'Partner drilling, focused on chain wrestling',
            'Technique review with coach',
            'Live rolling with teammates',
            'Position work from neutral'
        ],
        'Live Wrestling': [
            'Won by pin in first period',
            'Close match, lost by 2 points',
            'Scored multiple takedowns',
            'Got caught in a cradle',
            'Decision victory'
        ],
        'Drilling': [
            'Shot drills from neutral',
            'Bottom work and stand-ups',
            'Tie-up and dirty boxing',
            'Counter attacks',
            'Mat returns and scrambles'
        ],
        'Conditioning': [
            'Hill sprints and agility',
            'Mat endurance circuits',
    ...
    return noteOptions[type][Math.floor(Math.random() * noteOptions[type].length)];
}
